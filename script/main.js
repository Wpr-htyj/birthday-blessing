...
function showFullscreenBlessing() {
  ...
  const colors = [
    ...
    "#7bed9f", "#ff9ff3", "#54a0ff", "#ff6348"
  ]
  
  for (let i = 0; i < blessingCount; i++) {
    const blessing = document.createElement("div")
    blessing.className = "blessing-bubble"
    blessing.innerText = blessings[Math.floor(Math.random() * blessings.length)]
    
    blessing.style.left = Math.random() * 90 + "%"
    blessing.style.top = Math.random() * 90 + "%"
    blessing.style.backgroundColor = colors[Math.floor(Math.random() * colors.length)]
    blessing.style.zIndex = Math.floor(Math.random() * 100) + 1
    
    container.appendChild(blessing)
  }
  
  TweenMax.staggerTo(".blessing-bubble", 0.5, {
    opacity: 1,
    scale: 1,
    y: 0,
